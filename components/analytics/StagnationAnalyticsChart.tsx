...
const BAR_COLOR = '#f43f5e' // rose-500
      ...
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <YAxis
            ...
            tickFormatter={(v) => `${v}%`}
            domain={[0, 'auto']}
            label={{ value: '不良在庫割合', angle: 90, position: 'insideRight', style: { fontSize: 10, fill: AXIS_COLOR } }}
          />
          <Tooltip
            contentStyle={{
            ...
            formatter={(value: number, name: string) => {
              if (name === 'count') return [`${value}台`, '滞留台数']
              return [`${value}%`, '不良在庫割合']
            }}
          ...
          <Legend
            ...
            formatter={(value: string) =>
              value === 'count' ? '滞留台数（棒）' : value === 'ratio' ? '不良在庫割合（線）' : value
            }
          />
          <Bar
            yAxisId="left"
